/*Код программы InConvertInText - чтение и вывод на экран информации из трех источников: файла на диске, интернет-страницы и массива данных типа byte.*/
import java.io.*;
import java.net.URL;
public class InConvertInText {
    public static void readAllByByte( Reader in) throws IOException { while (true) {        int oneByte = in.read(); // читает 1 байт
    ...
    public static void main(String[] args) {
        try { // С потоком связан файл
            ...
            readAllByByte(rFile); System.out.print("\n\n\n"); inFile.close(); rFile.close(); // С потоком связана интернет-страница, передается «русская» кодировка
